...
let ReportsService = class ReportsService {
    ...
    async getFleetOverview() {
        ...
        const positions = (await Promise.all(keys.map((k) => this.redis.getJson(k)))).filter(Boolean);
        ...
        const summaries = await Promise.all(positions.map(async (pos) => {
    ...
    async getTripReport(vehicleId, from, to) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            if (p.status === "MOVING") {
                ...
                if (i > 0) {
                    distanceKm += haversineKm(Number(points[i - 1].lat), Number(points[i - 1].lon), Number(p.lat), Number(p.lon));
                }
            }
            else {
                stoppedTicks++;
        ...
        const totalMinutes = Math.round((points[points.length - 1].timestamp.getTime() - points[0].timestamp.getTime()) / 60000);
        return {
            vehicleId,
    ...
    async getSpeedDistribution(vehicleId, from, to) {
        const points = await this.prisma.trackingPoint.findMany({
            where: { vehicleId, timestamp: { gte: from, lte: to } },
            select: { speed: true },
        });
    ...
    calcDistance(points) {
        ...
        for (let i = 1; i < points.length; i++) {
            total += haversineKm(Number(points[i - 1].lat), Number(points[i - 1].lon), Number(points[i].lat), Number(points[i].lon));
        }
        return total;
    }
};
