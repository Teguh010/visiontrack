...
export default function AnalyticsPage() {
  ...
  // Stats
  const maxSpeed  = points.length ? Math.max(...points.map((p) => Number(p.speed))) : 0;
  const avgSpeed  = points.length ? points.reduce((s, p) => s + Number(p.speed), 0) / points.length : 0;
  ...
  return (
    <div className="flex flex-col h-screen p-6 gap-4 overflow-y-auto">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">Speed profile & stop detection per vehicle</p>
      </div>
      ...
      {points.length > 0 && (
        <div className="grid grid-cols-3 gap-3 flex-shrink-0">
          {[
            { label: "Max Speed",    value: `${maxSpeed.toFixed(1)} km/h`,  color: "blue"  },
            { label: "Avg Speed",    value: `${avgSpeed.toFixed(1)} km/h`,  color: "green" },
      ...
      {points.length > 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                ...
                <ReferenceLine y={5} stroke="#f97316" strokeDasharray="4 4"
                  label={{ value: "Stop threshold", fill: "#f97316", fontSize: 10 }} />
                <Line dataKey="speed" stroke="#3b82f6" strokeWidth={2}
                  dot={false} activeDot={{ r: 4 }} name="Speed" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-gray-600 py-20">
          ...
          <p className="text-sm">Select a vehicle and date range, then click "Analyze"</p>
        </div>
      )}
    </div>
  );
